...
def calcular_canvas(H, img1, img2):
    ...
    O canvas engloba todos esses pontos 
    ...
    # Projeta cada canto da img2 para o espaço da img1 usando H_inv 
    ...
    y_max = np.ceil(todos[:, 1].max()).astype(int)
     ...
     # off set se x_min < 0, precisamos deslocar tudo em abs(x_min) para a direita
...
def warpar_img2(H, img2, canvas_size, offset):
    ...
    Para cada pixel (x, y) do canvas, calcula de onde ele veio na img2 aplicando H_inv. assim nao tem buracos no canvas(pixels pretos).

    Parâmetros:
      H : (img1 em img2)
      img2 : imagem original 
    ...
    coords_canvas = np.stack([xs, ys, ones], axis=0)  

    #  Aplica H para mapear canvas (espaço img1) em img2 
    # H mapeia img1 em img2, então usamos diretamente (sem inversa)
...
def montar_panorama(img1, img2_warpada, offset):
    ...
    O canvas começa com img2 já distorcida 
    A região de img1 é sobrescrita com img1 original, imag1 semrpe ganah e fica por cima. img2 preenche apenas o que img1 não cobre.
    """
...
def recortar_panorama(panorama):
    ...
    for (cx, cy) in [(0, 0), (w-1, 0), (0, h-1), (w-1, h-1)]:
        if mascara_preta[cy, cx] == 255:         
            cv2.floodFill(mascara_preta, fundo, (cx, cy), 128)

    # pixels marcados com 128 = vira preto total
    # pixels que ficaram 255 = preto dentro da imagem 
